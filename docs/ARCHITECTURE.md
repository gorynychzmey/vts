...
**The pipeline stays strictly sequential — diarization is deliberately not run in parallel with transcription.** The DAG could support it: `diarize` reads the whole trimmed audio, not the transcript, so it has no real dependency on `transcribe_segments` and only follows it by position in the step list; `merge_transcript` already waits on both artefacts and is therefore already the join point. The two also use different compute — Whisper on the Vulkan GPU, pyannote on CPU — so they would not contend.

The saving was measured on a 49-minute meeting: transcription 3m34s (GPU), diarization 24m24s (CPU). Running them concurrently hides the shorter behind the longer, so the pair costs ~24m instead of ~27.5m — about **3.5 minutes, ~13% of that pair**, on a task whose total run is far longer.

That is not worth what it costs conceptually. `process_task` executes one step at a time, and every step's status, progress, and failure handling assumes that. A diamond in the DAG means concurrent branches, a barrier before the join, and a second failure mode to reason about (diarization is optional — if it fails the transcript must still reach the merge). It would also need its own resource lane, since diarization is CPU-bound and N parallel tasks must not start N diarizations on a 16-core box. Blurring "a task is a sequence of steps" for ~3.5 minutes is a bad trade.
