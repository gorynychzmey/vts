...
3. Прогресс-бар: **determinate-заливка**, переиспользуя существующее кольцо
   (`submit-progress`, `setProgress(ratio)` — то же, что у `uploadFileWithProgress`).
   После каждого успешного PATCH вызываем `setProgress(received / total_size)`,
   где `received` — подтверждённый сервером ответ `{received}`. Кольцо плавно
   заполняется реальными байтовыми долями (~`total_size / chunk_bytes` обновлений).
   Это честнее старого single-shot пути: тот использует `xhr.upload.onprogress`
   (прогресс браузер→прокси), который на больших файлах врёт из-за буферизации
   фронтящего прокси. При chunked каждый шаг подтверждён backend'ом. Кольцо НЕ
   indefinite-крутящееся — оно determinate с самого начала (`total_size` известен
   на init). На старте chunked-загрузки сбрасываем кольцо в 0 и сразу режим
   determinate.
...
- UI verifier: стаб `/api/uploads/*`; сценарий, что при файле > порога клиент
  ...
  boot и порог-ветвление выбирает правильный путь по `file.size`. Если достижимо в
  стабе — проверить, что прогресс-кольцо в chunked-режиме determinate (растёт по
  мере PATCH-ответов), а не висит indefinite.
