...
4. Каждый адаптер объявляет: `name`, `contract_version=(major,minor)`, entry point в группе `vts.delivery`.
   **`config` может приходить СМЁРЖЕННЫМ из нескольких источников** (учётка + таргет — см. vts-929).
   Адаптер получает плоский словарь и не должен полагаться на то, что все ключи заданы в одном месте;
   `config_schema()` валидируется по РЕЗУЛЬТАТУ мержа, а не по отдельным его частям. Зафиксировано до
   публикации контракта, чтобы наследование не стало ломающим сюрпризом для авторов плагинов.
