...
| 5 | Low | Open-redirect bypass via backslash and percent-encoded slash in `next` | [api/auth_routes.py:26-36](vts/api/auth_routes.py#L26-L36) — **fixed in 1.0.60 (vts-le1)** |
| 6 | Low | `as_user` query-param admin impersonation accepts arbitrary strings without normalisation | [services/auth.py:80-99](vts/services/auth.py#L80-L99) — **fixed in 1.0.60 (vts-9kk)** |
...
Revised 2026-05-25 after discussion: vts is published self-hosted, so
requiring users to generate a secret manually before first start is a
UX barrier that leaves the deterministic blake2b fallback as the
default for anyone who skips the docs.

- **Autogenerate on first start.** New setting `session_secret_file`
  (default `/opt/vts/state/session_secret`). Resolution order at
  startup:
  1. `VTS_SESSION_SECRET` env — use as-is (for explicit / HA
     deployments that share a secret across hosts).
  2. Otherwise read `session_secret_file`; if missing, generate
     `secrets.token_hex(32)`, write with `0600` via `open(path, 'x')`
     to survive worker-startup races (loser catches `FileExistsError`
     and reads).
- **Remove the blake2b fallback entirely** — the deterministic mode
  must not exist as a default or as a fallback.
- **New volume** in the systemd unit: `-v /opt/vts/state:/opt/vts/state`
  (rw, 0700). Not `/opt/vts/config` (ro, would force breaking that
  invariant). Not a single `-v /opt/vts:/opt/vts` (gives container rw
  on prompts/tokenizers).
- **Document rotation semantics:** `rm /opt/vts/state/session_secret`
  + restart → all users logged out. Rotating `oauth_client_secret` no
  longer touches sessions (they're now independent).
- **Document HA caveat:** for multi-host deployments, set
  `VTS_SESSION_SECRET` explicitly and share across hosts; the
  per-host autogenerated file would produce mismatched cookies behind
  a load balancer.

See vts-mmt for the full implementation design.
...
- ~~**Low:** Harden `_safe_next` against `\` and `%2f` (Finding 5).~~ **Done in 1.0.60 (vts-le1).**
- ~~**Low:** Normalise `as_user` to `.strip().lower()` (Finding 6).~~ **Done in 1.0.60 (vts-9kk).**
